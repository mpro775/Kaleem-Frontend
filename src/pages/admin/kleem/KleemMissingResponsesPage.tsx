...
export default function KleemMissingResponsesPage() {
  ...
  const [edit, setEdit] = useState<{ open: boolean; row?: KleemItem }>({
    open: false,
  });
  ...
  const columns = useMemo<GridColDef<KleemItem>[]>(
    () => [
      ...
      { field: "manualReply", headerName: "رد يدوي", flex: 1, minWidth: 200 },
      {
        ...
        renderCell: (p) =>
          p.value ? (
            <Chip label="تم الحل" color="success" size="small" />
          ) : (
            <Chip label="مفتوح" color="warning" size="small" />
          ),
      },
      {
        field: "actions",
        ...
        renderCell: (p) => (
          <Stack direction="row" spacing={1}>
            <Button
              size="small"
              onClick={() => setEdit({ open: true, row: p.row })}
            >
              تعديل
            </Button>
            {!p.row.resolved && (
              <Button
                size="small"
                onClick={async () => {
                  const updated = await updateKleem(p.row._id, {
                    resolved: true,
                  });
                  setRows((r) =>
                    r.map((x) => (x._id === updated._id ? updated : x))
                  );
                }}
              >
                حلّ
  ...
  return (
    <Stack gap={2}>
      ...
      {/* جدول */}
      <Paper className="p-2" style={{ height: 580 }}>
        <DataGrid
          rows={rows}
          columns={columns}
          getRowId={(r) => r._id}
      ...
      >
          ...
          <Stack gap={2} sx={{ mt: 1 }}>
            <TextField
              ...
              onChange={(e) =>
                setEdit((s) => ({
                  ...s,
                  row: { ...(s.row as KleemItem), manualReply: e.target.value },
                }))
              }
              multiline
            />
            <TextField
              ...
              onChange={(e) =>
                setEdit((s) => ({
                  ...s,
                  row: { ...(s.row as KleemItem), category: e.target.value },
                }))
              }
            />
          </Stack>
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              const r = edit.row!;
              const updated = await updateKleem(r._id, {
                manualReply: r.manualReply,
                category: r.category,
                resolved: r.resolved,
              });
              setRows((list) =>
