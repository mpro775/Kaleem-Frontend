...
export default function CartDialog({
  ...
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: سلة الشراء, 2: تفاصيل الشحن, 3: تأكيد الطلب
  ...
  const handleOrder = async () => {
    if (demo) {
      // إنشاء طلب وهمي وتخزينه محلياً (باستخدام بيانات العميل الفعلية للسلاسة)
    ...
    const products = items.map(({ product, quantity }) => ({
      product: product._id, // المفتاح المطلوب في الباك
      name: product.name,
      quantity,
      price: product.price,
    }));
              ...
              <React.Fragment key={s}>
                <Box
                  sx={{
                    ...
                    justifyContent: "center",
                    backgroundColor:
                      step >= s ? theme.palette.primary.main : "white",
                    border: `2px solid ${
                      step >= s
